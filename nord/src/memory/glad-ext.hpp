...
#pragma region function_handling
    ...
    std::vector< uint8_t > get_function_vector(
        uint8_t* functor,
        uintptr_t write_address,
        std::vector< naked_arg >& args,
        uintptr_t call_address = 0 );

    template< class C >
    C wait_for_ret( const ext_sub func )
    {
        const auto h_thread = CreateRemoteThread(
            handle_, nullptr, 0, reinterpret_cast< LPTHREAD_START_ROUTINE >( func.extern_adr ), nullptr, 0, nullptr );
    ...
    ext_sub map_function(
        R( __stdcall* functor )( Args... ),
        std::vector< naked_arg > args,
        const uintptr_t call_address = 0 )  // ensures __stdcall (needed because of callee/caller cleanup) (thanks kowalski)
    {
        ...
        return { func_alloc,
                 get_write_ret_adr( reinterpret_cast< uint8_t* >( functor ), func_alloc ),
                 reinterpret_cast< uint8_t* >( functor ) };
    }
...
#pragma region function helpers
    ...
    bool success()
    {
        return base_address_ && module_end_ && handle_;
    }
