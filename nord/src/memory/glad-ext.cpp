...
#pragma region function_handling
    ...
    std::vector< uint8_t > xg_process::get_function_vector(
        uint8_t * functor, const uintptr_t write_address, std::vector< naked_arg >& args, uintptr_t call_address )
    {
        const auto size = sizeof_function( functor );
...
#pragma region signature_scanning
    ...
    uintptr_t xg_process::sig_scan( const char* sig, const char* mask ) const
    {
        ...
        for ( auto base = base_address_; base < max; base += si_.dwPageSize - strlen( sig ) )
        {
            ...
            if ( ReadProcessMemory( handle_, reinterpret_cast< LPCVOID >( base ), &preload[ 0 ], preload.size(), nullptr ) ==
                 0 )
            {
#ifdef _DEBUG
            ...
            const auto ret = find_pattern(
                preload.data(),
                preload.size(),
                reinterpret_cast< uint8_t* >( const_cast< char* >( sig ) ),
...
#pragma region function helpers
    ...
    uintptr_t xg_process::copy_fn( uintptr_t address )  // credits to eternal, https://github.com/EternalV3/Retcheck
    {
        ...
        do
        {
            ...
            !( read< uint8_t >( address ) == 0x55 && read< uint8_t >( address + 1 ) == 0x8B &&
               read< uint8_t >( address + 2 ) == 0xEC ) );
...
#pragma region calling convention detection
    ...
    [[nodiscard]] xg_process::calling_convention xg_process::get_calling_convention( const uintptr_t adr )
    {
        ...
        for ( auto base = base_address_; base < max;
              base += si_.dwPageSize - 5 )  // again, size of rel call, page boundaries
        {
            std::vector< uint8_t > preload;
            preload.resize( si_.dwPageSize );
            if ( ReadProcessMemory( handle_, reinterpret_cast< LPCVOID >( base ), &preload[ 0 ], preload.size(), nullptr ) ==
            ...
            for ( unsigned int i = 0; i < preload.size(); i++ )
            {
                if ( preload.at( i ) == 0xE8 && i != preload.size() )
                {
                    ...
                    {
                        if ( preload.size() > i + 9 && preload.at( i + 8 ) == 0x83 &&
                             preload.at( i + 9 ) == 0xC4 )  // add esp, unk (stack cleanup, cdecl)
                        {
                            return cdecl_;
                        }
                    }
        ...
        if ( read< uint8_t >( adr ) != 0x55 && read< uint8_t >( adr + 1 ) != 0x8B &&
             read< uint8_t >( adr + 2 ) != 0xEC )  // msvc function
        {
            return error;
        }
        ...
        for ( sz; sz < INT_MAX; sz++ )
        {
            ...
            if ( one == 0x0 && two == 0x0 && three == 0x0 && four == 0x0 &&
                 five == 0x0 )  // copied function (from retcheck bypass)
                break;

            if ( one == 0xCC && two == 0xCC && three == 0xCC && four == 0xCC &&
                 five == 0xCC )  // copied function (from retcheck bypass)
                break;
        }
        ...
        for ( auto i = 0; i < sz; i++ )
        {
            ...
            if ( read< uint8_t >( adr + i ) == 0x5D && read< uint8_t >( adr + i + 1 ) == 0xC2 &&
                 read< uint8_t >( adr + i + 2 ) != 0 )
                return stdcall_;
        }
        return error;
    }
