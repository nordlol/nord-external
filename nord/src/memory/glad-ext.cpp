...
[[nodiscard]] std::string xg_process::read_str( const uintptr_t address )
{
    ...
    if ( read< uint8_t >( address + 4 ) == 0 && read< uint8_t >( address + 3 ) != 0 && read< uint8_t >( address + 2 ) != 0 &&
         read< uint8_t >( address + 1 ) != 0 )  // str ptr
...
std::vector< uint8_t > xg_process::get_function_vector(
    uint8_t* functor,
    const uintptr_t write_address,
    std::vector< naked_arg >& args,
        ...
        if ( functor[ i ] == 0xce && functor[ i + 1 ] == 0xcc && functor[ i + 2 ] == 0xcc && functor[ i + 3 ] == 0xcc )
        {
            ...
            for ( auto val : args )
            {
                if ( std::holds_alternative< uintptr_t >( val ) )
                {
                    ...
                    // 68			01+					PUSH	imm16/32										Push Word, Doubleword or Quadword Onto the
                    // Stack
                ...
                else if ( std::holds_alternative< bool >( val ) )
                {
                    ...
                    // 6A			01 + PUSH	imm8										Push Word, Doubleword or Quadword Onto the
                    // Stack
