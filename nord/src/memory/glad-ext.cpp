                ...
                if ( entry.cntThreads > 20 )
                {
                    ...
                    do
                    {
...
#endif
                        {
                            ...
                            printf( "process id %d\n", pid_ );
                            printf( "process base_address_ %u\n", base_address_ );
                            printf( "process module_end_ %ul\n", module_end_ );
                            break;
                        }
...
uintptr_t xg_process::copy_fn( uintptr_t address )  // credits to eternal, https://github.com/EternalV3/Retcheck
{
    ...
    for ( size_t i = 0; i < sz; i++ )  // emulate memcpy ig lmao
...
[[nodiscard]] xg_process::calling_convention xg_process::get_calling_convention( const uintptr_t adr )
{
        ...
        for ( unsigned int i = 0; i < preload.size(); i++ )
        {
            if ( preload.at( i ) == 0xE8 && i != preload.size() )
            {
                ...
                if ( base + i + rel_offset + 5 == adr )
                {
                    ...
                    {
                        return calling_convention::cdecl_;
                    }
                }
            }
        }
    ...
    {
        return calling_convention::error;
    }
    ...
    for ( auto i = 0; i < sz; i++ )
    {
        ...
        if ( read< uint8_t >( adr + i ) == 0x5D && read< uint8_t >( adr + i + 1 ) == 0xC2 &&
            ...
            return calling_convention::stdcall_;
    }
    return calling_convention::error;
}
